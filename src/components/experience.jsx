...
export default function Experience({ scrollLeft, pageno, data }) {
  ...
  const { select, updateArray, deleteArray, dataUpdateArray, mode } = alldata(
    state => {
      return {
            ...
            <textarea
              ...
              className={`${
                mode === "dark"
                  ? " text-white bg-gray-700 border-zinc-600 focus:border-zinc-400"
                  : "text-black border-indigo-400 focus:border-indigo-600 "
              } w-full border  p-[8px] outline-none 0 rounded-md font-bold `}
            />
          </div>
        </div>
      ) : null}
      <div className=' flex-wrap flex justify-evenly md:justify-normal  flex-row-reverse p-3'>
        ...
        <div className=' text-xs md:text-base md:w-[300px] p-2'>
          {load ? (
            ...
            <div
              onClick={() => {}}
              className=' text-xs md:text-base  p-2 flex justify-start items-start'>
              {load ? (
                <div>
                  <Load />
                </div>
